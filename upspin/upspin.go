...
type BlockUnpacker interface {
	...
	// into dst and if appropriate the result is verified as correct according
	// to the block's Packdata.
	//
	// If len(dst) < len(ciphertext) UnpackBlock should panic.
	//
	// UnpackBlock is not affected by NextBlock.
	UnpackBlock(dst, ciphertext []byte, n int) error
